...
sub sql_do_update {
	...
	# При передаче пустой строки в численное поле генерируется ошибка преобразования типов
...
sub assert_view {
	...
	$self -> do ("IF EXISTS (SELECT TABLE_NAME FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_NAME = '$name') DROP VIEW $name");
